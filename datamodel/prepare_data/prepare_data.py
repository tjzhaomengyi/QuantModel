...
def split_min_data(row, tag, start_index, end_index):
...
data = pd.read_csv('/home/zhaomengyi/Projects/QuantProjects/QuantDataMiner/datamodel/prepare_data/data_full_info.csv',
                   escapechar=None, converters={'min_info':parse_ordered_dict_array})
# data['min_info'] = data['min_info'].apply(parse_ordered_dict_array)
#3、将min_info分成8组，每3个一组
for i in range(8):
    prc_tag = 'min_prc_' + str(i)
    vol_tag = 'min_vol_' + str(i)
    start_index = i * 30
    end_index = i * 30 + 29
    data[prc_tag] = data.apply(lambda row:split_min_data(row, 'price', start_index, end_index), axis=1)
    data[vol_tag] = data.apply(lambda row:split_min_data(row, 'vol', start_index, end_index), axis=1)

data = data.drop('min_info', axis=1)

#data['day_info']的数据类型时字典，key为日线上各种指标，value是近四天指标的具体数值
...
data = data.drop('day_info', axis=1)
